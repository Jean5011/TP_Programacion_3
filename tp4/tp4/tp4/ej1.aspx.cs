...
namespace tp4
{
    public partial class ej1 : System.Web.UI.Page
    {
        ...
        protected void Destino()
        {
            int provicia = int.Parse(DDL_Inicio_Provicia.SelectedValue);
            SqlConnection cn = new SqlConnection("Data Source=localhost\\sqlexpress;Initial Catalog=Viajes;Integrated Security=True");
            DataSet ds = new DataSet();
            cn.Open();
            SqlDataAdapter adpt = new SqlDataAdapter("Select * from [Provincias] where [IdProvincia] != '" + (provicia) + "'", cn);
            adpt.Fill(ds, "localidades");
            DDL_Destino_Provicia.DataSource = ds.Tables["Localidades"];
            DDL_Destino_Provicia.DataTextField = "NombreProvincia";
            DDL_Destino_Provicia.DataValueField = "IdProvincia";
            DDL_Destino_Provicia.DataBind();
        ...
        {
            DDL_Destino_Localidad.Items.Clear();
            int provicia = int.Parse(DDL_Destino_Provicia.SelectedValue);
            SqlConnection cn = new SqlConnection("Data Source=localhost\\sqlexpress;Initial Catalog=Viajes;Integrated Security=True");
            DataSet ds = new DataSet();
            cn.Open();
            SqlDataAdapter adpt = new SqlDataAdapter("Select * from [Localidades] where [IdProvincia] = '" + (provicia) + "'", cn);
            adpt.Fill(ds, "localidades");
            DDL_Destino_Localidad.DataSource = ds.Tables["Localidades"];
            DDL_Destino_Localidad.DataTextField = "NombreLocalidad";
            DDL_Destino_Localidad.DataValueField = "IdLocalidad";
            DDL_Destino_Localidad.DataBind();
            cn.Close();
        }*/
    }
}
